...
因此，我们接下来的重点，就是来看看 `for` 循环中究竟发生了什么。为了格式上简单一些，接下来的代码部分，我就不再列出 `vsprintf` 外围的部分了。在 `for` 循环的一开始，我们处理最简单的情况，即模版中的内容，就是要显示的字符，我们直接把字符拷贝到 `str` 就好了：
...
说到这，内核打印日志的函数库基本就完成了，有了这些 API，我们就又能在屏幕上显示内容了。可以在图形模式下显示内容，这绝对是内核开发中意义重大的一步。接下来，我们为内核实现中断和异常处理模块。

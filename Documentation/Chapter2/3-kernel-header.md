...
* AT&T 语法的操作数顺序是从左向右，第一个操作数是源，第二个操作数是目的。这和 Intel 语法是完全相反的；
* AT&T 语法要在立即数前面加 `$`，例如 `$0x10`，`$0x7E00`；
* AT&T 语法要在寄存器要在前面加 `%`，例如 `%ax`；
...
它只包含了一条有用的记录，索引位置是 0。因此，只有线性地址 bit 20-38 为 0 的地址是可访问的。其余的表项，我们用 `.fill` 填充成 0。通过页目录指针不难发现，页目录的地址，安排在了 `0x103000`。因此，我们用 `org 0x3000` 定义页目录：
